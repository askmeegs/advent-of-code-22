...
def main(): 
    ...
    caveBottom = 0  # max Y val 
    # draw rock paths in grid 
    # a rock path looks like: 
    # 503,4 -> 502,4 -> 502,9 -> 494,9 

    # so "0" = minX, 494, and len = maxX, 503 
    # and you're going down from 0 to maxy (9)
    ...
    while True: 
        ...
        # attempt to move 
        # first, check if we're about to enter the void, if so BREAK 
        if x >= maxX or y < 0 or y-minY > len(grid[0]):
            break
        # look down 
        elif grid[x+1][y] == ".":
            x += 1 
