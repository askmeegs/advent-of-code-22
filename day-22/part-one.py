def main(): 
    # instructions - parse 
    instr = "10R5L5R10L4R5L5"
    # instr = ""
    instr_list = [] 
    i = 0 
    while i < len(instr):
        char = instr[i]
        # if char is a number..
        if char.isdigit():
            # look ahead for more digits
            j = i+1 
            while j < len(instr) and instr[j].isdigit():
                char += instr[j]
                j += 1
            instr_list.append(int(char))
            i = j
            continue 
        else:
            instr_list.append(char)
            i += 1
  
    # populate map 
    with open('small.txt', 'r') as f:
        lines = f.readlines()

    """
    # = wall 
    . = open 
    x = void 

    > = I was here, facing right
    < = I was here, facing left
    v = I was here, facing down
    ^ = I was here, facing up 
    """

    # create map
    map = []
    # what is the longest line? 
    lens = [len(line) for line in lines]
    max_row_length = max(lens)


    for line in lines:
        row = []
        i = 0 
        for char in line:
            if char == '#':
                row.append('#')
            elif char == '.':
                row.append('.')
            else:
                row.append('x')
            i += 1 
        if i > max_row_length:
            max_row_length = i
        if i < max_row_length:
            # pad the row with "x"
            while i < max_row_length:
                row.append('x')
                i += 1
        map.append(row)
    pretty_print(map)

    # dimensions
    print("There are {} rows in the map".format(len(map)))
    for row in map:
        print(len(row))

    # start at top left, facing right
    # facing: 0 for right (>), 1 for down (v), 2 for left (<), and 3 for up (^)

    row = 0
    col = 0
    facing = 0
    # figure out the real starting point - the first "." tile in row 1 
    for i in range(len(map[0])):
        if map[0][i] == '.':
            col = i
            break


    # execute instructions
    for instr in instr_list: 
        print("👋🏼 New instruction: {}".format(instr))
        # pretty_print(map)
        # if instr is to rotate, don't move, just update mark in position 
        if instr == 'R':
            facing = (facing + 1) % 4
            map[row][col] = ['>', 'v', '<', '^'][facing]
            continue 
        if instr == 'L':
            facing = (facing - 1) % 4
            map[row][col] = ['>', 'v', '<', '^'][facing]
            continue 

        # otherwise, assume instr is a number of steps to move forward. 
        for i in range(instr): 
            print("👟 I'm facing {}. Moving 1 step within instruction {}, (on step #{})".format(facing, instr, i))
            # mark where I am NOW because I might be about to move  
            map[row][col] = ['>', 'v', '<', '^'][facing] 
            # pretty_print(map)
            
            should_move = False 
            exit_instr = False

            # RIGHT 
            if facing == 0: 
                tr = row 
                tc = col+1 
            # DOWN 
            elif facing == 1:
                tr = row+1 
                tc = col 
            # LEFT 
            elif facing == 2:
                tr = row
                tc = col-1 
            # UP 
            else: 
                tr = row-1 
                tc = col 

            # determine behavior 
            # Case A: I've fallen into the void or out of bounds 
            # Case A1: wrap forward in same row (end of list to 0) 
            """
            Facing is 0 for right (>), 1 for down (v), 2 for left (<), and 3 for up (^).
            """
            # RIGHT 
            if facing == 0 and (tc >= len(map[0]) or map[tr][tc] == "x"):
                tc = 0
                while map[tr][tc] == "x":
                    tc += 1 
            # LEFT 
            if facing == 2 and (tc < 0 or map[tr][tc] == "x"):
                tc = len(map[0])-1 
                # print("I'm left. About to look at tr={} tc={}".format(tr, tc)) 
                # print("There are #{} items in row {}.".format(len(map[tr]), tr))
                z = 0 
                while map[tr][tc] == "x":
                    print(z)
                    z += 1 
                    tc -= 1
            # UP 
            if facing == 3 and (tr < 0 or map[tr][tc] == "x"):
                tr = len(map)-1 
                while map[tr][tc] == "x":
                    tr -= 1 
            # DOWN 
            if facing == 1 and (tr >= len(map) or map[tr][tc] == "x"):
                tr = 0 
                while map[tr][tc] == "x":
                    tr += 1 


            # CHECK FOR ALL CASES INCLUDING WRAP... 
            # Case B: I'm in bounds but I've hit a wall 
            if map[tr][tc] == "#":
                exit_instr = True
            # Case C: I'm in bounds and the next spot is open or already marked. (Move to it.)
            elif map[tr][tc] in [".", ">", "<", "^", "v"]:
                should_move = True 
            else:
                print("🚨 unexpected value. tr={}, tc={}, val={}, mayday mayday".format(tr, tc, map[tr][tc]))
                exit()
            
            if exit_instr == True:
                print("❌ I hit a wall. stop moving.")
                break 

            # if I should move, move, but don't mark. 
            if should_move == True:
                row = tr 
                col = tc 
            


    
    # print final position
    print("FINAL MAP: ")
    pretty_print(map)
    print("\n")

    # 1-indexed 
    row += 1
    col += 1

    print("Final position: ({}, {})".format(row, col))
    print("Final facing: {}".format(facing))

    # print password
    password = (1000 * row) + (4 * col) + facing
    print("🏁 DONE: Password: {}".format(password))


def pretty_print(map):
    print("\n")
    for row in map:
        print(''.join([' ' if char == 'x' else char for char in row]))
    print("\n")
# execute main
if __name__ == '__main__':
    main()